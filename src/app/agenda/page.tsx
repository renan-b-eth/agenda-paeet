...
export default function AgendaPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      {/* Week Navigation */}
      <div className="flex items-center justify-center gap-4 py-4 border-b bg-zinc-50 dark:bg-zinc-900">
        <button
      ...
      <main className="flex-1 p-4 md:p-6 max-w-6xl mx-auto w-full">
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  <th className="border p-2 bg-zinc-100 dark:bg-zinc-800 text-sm font-semibold w-20">
                    Horário
                  </th>
                  {DAYS.map((day) => (
                    <th
                      key={day}
                      className="border p-2 bg-zinc-100 dark:bg-zinc-800 text-sm font-semibold"
                    >
                      {day}
                    </th>
                  ))}
              ...
              <tbody>
                {PERIODS.map((period) => (
                  <tr key={period}>
                    <td className="border p-2 bg-zinc-50 dark:bg-zinc-900 text-sm font-medium text-center">
                      {period}
                    </td>
                    {DAYS.map((day) => {
                      ...
                      return (
                        <td
                          key={`${day}-${period}`}
                          className="border p-2 min-h-[80px] align-top"
                        >
                          {slot ? (
                            <div>
        ...
        <div className="mt-6 text-xs text-zinc-500 text-center">
          {slots.length === 0 && !loading && (
            <p>Nenhum slot cadastrado para esta semana. Use a API para criar slots.</p>
          )}
        </div>
      </main>
    </div>
